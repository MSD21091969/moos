...
// applyAdd: ∅ → Container. Creates a new Node in Ob(C).
...
// applyLink: C × C → Wire. Creates a new Wire in Hom(C).
...
// applyMutate: C → C. Updates a Node's payload (endomorphism with version CAS).
...
// applyUnlink: Wire → ∅. Removes a wire from Hom(C).
...
// deepCopy clones a map[string]any tree. Shared with cat/state.go pattern.
