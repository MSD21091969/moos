...
- [x] Kill kernel → restart → verify all sessions recover
- [x] Kill postgres → restart → verify data intact
- [x] Corrupt morphism log → verify kernel detects and reports (but doesn't crash)
*(Note: Redis session dependency removed in Phase 4; session persistence relies entirely on Postgres Universal Graph Model).*

### Quality Gate

- [x] `docker compose up` from clean state works in <60 seconds
- [x] Full conversation works end-to-end in Docker
- [x] Restart kernel: zero data loss
- [x] Prometheus metrics scraping works
- [x] JSON logs parseable by standard log aggregators
- [x] No TypeScript runtime services remain (only surfaces)
- [x] README enables new developer to get running in <15 minutes
- [x] `golangci-lint`, `buf lint`, `eslint` all pass in CI
- [x] Docker image CVE scan: zero critical/high vulnerabilities
