...
func Materialize(req MaterializeRequest, registry *operad.Registry, dryRun bool) (MaterializeResult, error) {
	...
	for _, nr := range req.Nodes {
		if isGlossaryURN(nr.URN) {
			validationErrors = append(validationErrors, fmt.Sprintf("node %s: glossary category nodes are not materializable", nr.URN))
			continue
		}
		if nr.Kind == "Kernel" || nr.Kind == "Feature" {
			validationErrors = append(validationErrors, fmt.Sprintf("node %s: legacy kinds Kernel/Feature are not materializable", nr.URN))
			continue
		}
		if isLegacySystemURN(nr.URN) {
			validationErrors = append(validationErrors, fmt.Sprintf("node %s: legacy system node is not materializable", nr.URN))
			continue
		}
	...
	for _, wr := range req.Wires {
		if isGlossaryURN(wr.SourceURN) || isGlossaryURN(wr.TargetURN) {
			validationErrors = append(validationErrors, fmt.Sprintf("wire %s→%s: glossary category wires are not materializable", wr.SourceURN, wr.TargetURN))
			continue
		}
		if isLegacySystemURN(wr.SourceURN) || isLegacySystemURN(wr.TargetURN) {
			validationErrors = append(validationErrors, fmt.Sprintf("wire %s→%s: legacy system wires are not materializable", wr.SourceURN, wr.TargetURN))
			continue
		}
...
func isGlossaryURN(urn string) bool {
	return strings.HasPrefix(urn, "urn:moos:cat:")
}

func isLegacySystemURN(urn string) bool {
	if strings.HasPrefix(urn, "urn:moos:feature:") {
		return true
	}
	return urn == "urn:moos:kernel:wave-0"
}
