...
# 1. Provide containerStore interface near type Manager
text = text.replace(
'''type Manager struct {
...
text = text.replace("return NewManagerWithStore(executor, dispatcher, ttl, cleanupEvery, logger, nil)", "return NewManagerWithContainerStore(executor, dispatcher, ttl, cleanupEvery, logger, nil)")
...
with open(path, "w", encoding="utf-8") as f:
    f.write(text)
print("final clean refactor done")
